...
integration('social repository local postgres',()=>{
 ...
 it('projects bounded whole root groups and preserves a deleted root tombstone',async()=>tx(async c=>{ const actor=await human(c),author=await ip(c),postId=await post(c,author),social=repo(c); const firstRoot=await social.createHumanComment(actor,postId,{body:'first'},context()); const reply=await social.createHumanComment(actor,postId,{body:'reply',parentCommentId:firstRoot.id},context()); const secondRoot=await social.createHumanComment(actor,postId,{body:'second'},context()); await c.query("UPDATE public.comments SET state='deleted',deleted_at=clock_timestamp() WHERE id=$1",[firstRoot.id]); const first=await social.getPost({viewer:null,postId,commentLimit:1,commentAfter:null}); expect(first?.comments.nextCursor).toBeTruthy(); const {decodeCommentCursor}=await import('@aifans/contracts'); const after=decodeCommentCursor(first!.comments.nextCursor!,postId); const second=await social.getPost({viewer:null,postId,commentLimit:1,commentAfter:after}); const groups=[...(first?.comments.groups??[]),...(second?.comments.groups??[])]; expect(groups.map(group=>group.root.id).sort()).toEqual([firstRoot.id,secondRoot.id].sort()); const tombstone=groups.find(group=>group.root.id===firstRoot.id)!; expect(tombstone).toEqual({root:expect.objectContaining({id:firstRoot.id,state:'deleted',author:null,rootCommentId:firstRoot.id}),replies:[expect.objectContaining({id:reply.id,parentCommentId:firstRoot.id,rootCommentId:firstRoot.id})]}); expect(tombstone.root).not.toHaveProperty('body'); expect(second?.comments.nextCursor).toBeNull() }))
 ...
 it('finds a deep-link target beyond the first fifty root groups',async()=>tx(async c=>{ const postAuthor=await ip(c),actor=await human(c),postId=await post(c,postAuthor),social=repo(c); for(let index=0;index<51;index+=1) await social.createHumanComment(actor,postId,{body:`earlier ${index}`},context()); const target=await social.createHumanComment(actor,postId,{body:'target root'},context()); const result=await social.getCommentThread({viewer:null,postId,commentId:target.id}); expect(result).toEqual({group:{root:expect.objectContaining({id:target.id,body:'target root'}),replies:[]}}) }))
