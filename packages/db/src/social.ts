...
export function createSocialRepository({withActor: runWithActor = withActor, withPublic = defaultPublicSession(defaultPool())}: {withActor?: WithActor; withPublic?: PublicSession} = {}): SocialRepository {
  ...
  return {
    ...
    async listNotifications(actor, page) { return runWithActor(actor, async (client) => { let afterId: string | null = null; if (page.cursor) { try { const value = JSON.parse(Buffer.from(page.cursor, 'base64url').toString('utf8')) as {v?: number; kind?: string; createdAt?: string; id?: string}; if (value.v !== 1 || value.kind !== 'notifications' || typeof value.createdAt !== 'string' || typeof value.id !== 'string') throw new Error(); afterId = value.id } catch { throw new Error('INVALID_CURSOR') } }; const result = await client.query<{id: string; kind: 'follow' | 'post_like' | 'comment' | 'reply' | 'comment_like'; post_id: string | null; comment_id: string | null; created_at: Date | string; read_at: Date | string | null; actor_id: string | null; actor_kind: 'human' | 'ip' | null; username: string | null; display_name: string | null; bio: string | null; languages: string[] | null}>('SELECT * FROM public.social_my_notifications($1,$2)', [afterId, page.limit + 1]); const rows=result.rows.slice(0,page.limit); const last=rows.at(-1); return NotificationPageSchema.parse({items: rows.map((row) => { const actor = !row.actor_id || !row.actor_kind || !row.username || !row.display_name ? null : row.actor_kind === 'human' ? {kind: 'human' as const, id: row.actor_id, username: row.username, displayName: row.display_name} : {kind: 'ip' as const, id: row.actor_id, username: row.username, displayName: row.display_name, bio: row.bio, languages: (row.languages ?? []) as Locale[]}; return {id: row.id, kind: row.kind, actor, postId: row.post_id, commentId: row.comment_id, createdAt: iso(row.created_at), readAt: row.read_at ? iso(row.read_at) : null} }), nextCursor: result.rows.length>page.limit&&last ? Buffer.from(JSON.stringify({v:1,kind:'notifications',createdAt:iso(last.created_at),id:last.id}),'utf8').toString('base64url') : null}) }) },
