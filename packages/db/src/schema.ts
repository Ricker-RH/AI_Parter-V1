...
export const creatorVisualTypeEnum = pgEnum('creator_visual_type', ['realistic', 'anime', 'hybrid'])
export const creatorDraftStateEnum = pgEnum('creator_draft_state', ['draft', 'submitted'])
export const creatorSubmissionStateEnum = pgEnum('creator_submission_state', ['pending_review', 'approved', 'rejected'])
export const creatorReferenceRoleEnum = pgEnum('creator_reference_role', ['avatar', 'cover', 'portrait', 'full_body', 'supporting_1', 'supporting_2', 'supporting_3', 'supporting_4'])
export const creatorRequestKindEnum = pgEnum('creator_request_kind', ['change', 'unpublish', 'deletion'])
export const creatorRequestStateEnum = pgEnum('creator_request_state', ['pending', 'approved', 'rejected'])
export const creatorDecisionValueEnum = pgEnum('creator_decision_value', ['approve', 'reject'])
...
}, (table) => [check('analytics_outbox_payload_version_check', sql`${table.payloadVersion} > 0`), check('analytics_outbox_destination_check', sql`${table.destination} ~ '[^[:space:]]'`), check('analytics_outbox_attempt_count_check', sql`${table.attemptCount} >= 0`), check('analytics_outbox_delivery_state_check', sql`(${table.state} = 'delivered') = (${table.deliveredAt} IS NOT NULL)`), check('analytics_outbox_lease_pair_check', sql`(${table.leaseToken} IS NULL) = (${table.leaseExpiresAt} IS NULL)`), check('analytics_outbox_terminal_lease_check', sql`${table.state} = 'pending' OR ${table.leaseToken} IS NULL`), check('analytics_outbox_error_code_length_check', sql`${table.lastErrorCode} IS NULL OR char_length(${table.lastErrorCode}) BETWEEN 1 AND 64`)])
...
export const creatorQuotas = pgTable('creator_quotas', {
  ...
  updatedAt: timestamp('updated_at', {withTimezone: true}).notNull().defaultNow(),
}, (table) => [check('creator_quotas_ip_quota_check', sql`${table.ipQuota} BETWEEN 0 AND 100`)])

export const creatorDrafts = pgTable('creator_drafts', {
  id: uuid().primaryKey(),
  creatorProfileId: uuid('creator_profile_id').notNull().references(() => profiles.id),
  targetIpProfileId: uuid('target_ip_profile_id').references(() => ipProfiles.profileId),
  ...
  check('creator_drafts_username_check', sql`${table.username} ~ '^[a-z0-9_]{3,30}$'`),
  check('creator_drafts_display_name_check', sql`char_length(${table.displayName}) BETWEEN 1 AND 80 AND ${table.displayName} ~ '[^[:space:]]'`),
  check('creator_drafts_short_description_check', sql`char_length(${table.shortDescription}) <= 500`),
  check('creator_drafts_language_codes_check', sql`cardinality(${table.languageCodes}) BETWEEN 1 AND 20`),
  check('creator_drafts_content_themes_check', sql`cardinality(${table.contentThemes}) BETWEEN 1 AND 12`),
  check('creator_drafts_personality_check', sql`char_length(${table.personality}) BETWEEN 1 AND 1000 AND ${table.personality} ~ '[^[:space:]]'`),
  check('creator_drafts_background_check', sql`char_length(${table.background}) BETWEEN 1 AND 2000 AND ${table.background} ~ '[^[:space:]]'`),
  check('creator_drafts_world_check', sql`char_length(${table.world}) BETWEEN 1 AND 2000 AND ${table.world} ~ '[^[:space:]]'`),
  check('creator_drafts_values_text_check', sql`char_length(${table.valuesText}) BETWEEN 1 AND 1000 AND ${table.valuesText} ~ '[^[:space:]]'`),
  check('creator_drafts_tone_check', sql`char_length(${table.tone}) BETWEEN 1 AND 500 AND ${table.tone} ~ '[^[:space:]]'`),
  check('creator_drafts_interests_check', sql`cardinality(${table.interests}) BETWEEN 0 AND 20`),
  check('creator_drafts_boundaries_check', sql`char_length(${table.boundaries}) BETWEEN 1 AND 1000 AND ${table.boundaries} ~ '[^[:space:]]'`),
...
export const creatorReferenceAssets = pgTable('creator_reference_assets', {
  ...
  unique('creator_reference_assets_draft_id_draft_role_key').on(table.draftId, table.draftRole),
  unique('creator_reference_assets_id_draft_id_key').on(table.id, table.draftId),
  check('creator_reference_assets_object_key_check', sql`char_length(${table.objectKey}) BETWEEN 1 AND 512`),
  check('creator_reference_assets_content_type_check', sql`${table.contentType} IN ('image/jpeg','image/png','image/webp')`),
  check('creator_reference_assets_width_check', sql`${table.width} BETWEEN 1 AND 16384`),
...
export const creatorRevisions = pgTable('creator_revisions', {
  ...
  unique('creator_revisions_draft_id_version_key').on(table.draftId, table.version),
  unique('creator_revisions_id_creator_profile_id_key').on(table.id, table.creatorProfileId),
  unique('creator_revisions_id_draft_id_key').on(table.id, table.draftId),
  unique('creator_revisions_id_draft_id_creator_profile_id_key').on(table.id, table.draftId, table.creatorProfileId),
  foreignKey({name: 'creator_revisions_draft_owner_fkey', columns: [table.draftId, table.creatorProfileId], foreignColumns: [creatorDrafts.id, creatorDrafts.creatorProfileId]}),
  check('creator_revisions_version_check', sql`${table.version} > 0`),
  check('creator_revisions_username_check', sql`${table.username} ~ '^[a-z0-9_]{3,30}$'`),
  check('creator_revisions_display_name_check', sql`char_length(${table.displayName}) BETWEEN 1 AND 80 AND ${table.displayName} ~ '[^[:space:]]'`),
  check('creator_revisions_short_description_check', sql`char_length(${table.shortDescription}) <= 500`),
  check('creator_revisions_language_codes_check', sql`cardinality(${table.languageCodes}) BETWEEN 1 AND 20`),
  check('creator_revisions_content_themes_check', sql`cardinality(${table.contentThemes}) BETWEEN 1 AND 12`),
])

export const creatorRevisionReferences = pgTable('creator_revision_references', {
  revisionId: uuid('revision_id').notNull().references(() => creatorRevisions.id),
  ...
  unique('creator_revision_references_revision_id_asset_id_key').on(table.revisionId, table.assetId),
  foreignKey({name: 'creator_revision_references_revision_id_draft_id_fkey', columns: [table.revisionId, table.draftId], foreignColumns: [creatorRevisions.id, creatorRevisions.draftId]}),
  foreignKey({name: 'creator_revision_references_asset_id_draft_id_fkey', columns: [table.assetId, table.draftId], foreignColumns: [creatorReferenceAssets.id, creatorReferenceAssets.draftId]}),
])

export const creatorIpRevisions = pgTable('creator_ip_revisions', {
  ipProfileId: uuid('ip_profile_id').notNull().references((): AnyPgColumn => ipProfiles.profileId),
  revisionId: uuid('revision_id').notNull().unique().references(() => creatorRevisions.id),
  creatorProfileId: uuid('creator_profile_id').notNull().references(() => profiles.id),
  createdAt: timestamp('created_at', {withTimezone: true}).notNull().defaultNow(),
}, (table) => [
  primaryKey({columns: [table.ipProfileId, table.revisionId]}),
  foreignKey({name: 'creator_ip_revisions_ip_owner_fkey', columns: [table.ipProfileId, table.creatorProfileId], foreignColumns: [ipProfiles.profileId, ipProfiles.creatorProfileId]}),
...
export const operatingAuthorizationAcceptances = pgTable('operating_authorization_acceptances', {
  ...
  foreignKey({name: 'operating_acceptances_draft_owner_fkey', columns: [table.draftId, table.creatorProfileId], foreignColumns: [creatorDrafts.id, creatorDrafts.creatorProfileId]}),
  foreignKey({name: 'operating_acceptances_revision_provenance_fkey', columns: [table.revisionId, table.draftId, table.creatorProfileId], foreignColumns: [creatorRevisions.id, creatorRevisions.draftId, creatorRevisions.creatorProfileId]}),
  check('operating_acceptances_authorization_version_check', sql`char_length(${table.authorizationVersion}) BETWEEN 1 AND 100 AND ${table.authorizationVersion} ~ '[^[:space:]]'`),
])
...
export const creatorSubmissions = pgTable('creator_submissions', {
  ...
  ipProfileId: uuid('ip_profile_id').unique().references(() => ipProfiles.profileId),
  submittedAt: timestamp('submitted_at', {withTimezone: true}).notNull().defaultNow(),
  decidedAt: timestamp('decided_at', {withTimezone: true}),
  decisionReason: text('decision_reason'),
}, (table) => [
  foreignKey({name: 'creator_submissions_draft_owner_fkey', columns: [table.draftId, table.creatorProfileId], foreignColumns: [creatorDrafts.id, creatorDrafts.creatorProfileId]}),
...
export const creatorSubmissionDecisions = pgTable('creator_submission_decisions', {
  id: uuid().primaryKey(),
  submissionId: uuid('submission_id').notNull().unique().references(() => creatorSubmissions.id),
  ...
  check('creator_submission_decisions_reason_check', sql`${table.reason} IS NULL OR char_length(${table.reason}) <= 2000`),
  check('creator_submission_decisions_decision_check', sql`${table.decision} = 'approve' OR (${table.decision} = 'reject' AND ${table.reason} IS NOT NULL AND ${table.reason} ~ '[^[:space:]]')`),
])

export const creatorIpRequests = pgTable('creator_ip_requests', {
  id: uuid().primaryKey(),
  ipProfileId: uuid('ip_profile_id').notNull().references(() => ipProfiles.profileId),
  creatorProfileId: uuid('creator_profile_id').notNull().references(() => profiles.id),
  kind: creatorRequestKindEnum('kind').notNull(),
  reason: text().notNull(),
  proposedRevisionId: uuid('proposed_revision_id').references(() => creatorRevisions.id),
...
export const creatorRequestDecisions = pgTable('creator_request_decisions', {
  id: uuid().primaryKey(),
  requestId: uuid('request_id').notNull().unique().references(() => creatorIpRequests.id),
  decision: creatorDecisionValueEnum('decision').notNull(),
  decidedByProfileId: uuid('decided_by_profile_id').notNull().references(() => profiles.id),
  reason: text(),
  correlationId: uuid('correlation_id').notNull(),
  decidedAt: timestamp('decided_at', {withTimezone: true}).notNull().defaultNow(),
}, (table) => [
  check('creator_request_decisions_reason_check', sql`${table.reason} IS NULL OR char_length(${table.reason}) <= 2000`),
  check('creator_request_decisions_decision_check', sql`${table.decision} = 'approve' OR (${table.decision} = 'reject' AND ${table.reason} IS NOT NULL AND ${table.reason} ~ '[^[:space:]]')`),
])
