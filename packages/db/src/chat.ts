...
type MessageRow = {
  ...
  in_reply_to_client_request_id?:string|null
  human_profile_id?: string
  ip_profile_id?: string
  provider_conversation_id?: string | null
  generation_state?: 'generating'|'partial'|'failed'|'completed'|null
  generation_answer?: string|null
}
...
export type ChatRepository = {
  ...
  checkpointProviderReply(actor: Actor,input:{conversationId:string;humanMessageId:string;answer:string}):Promise<boolean>
}
...
function message(row: MessageRow): ChatMessage {
  return ChatMessageSchema.parse({
    ...
    ...(row.generation_state ? {generation:{state:row.generation_state,answer:row.generation_answer??''}} : {}),
    ...(row.role==='human' && row.client_request_id ? {clientRequestId:row.client_request_id}:{}),
    ...(row.role==='assistant' && row.in_reply_to_client_request_id ? {inReplyToClientRequestId:row.in_reply_to_client_request_id}:{}),
  })
}
...
WHERE conversation.human_profile_id = $1::uuid AND conversation.ip_profile_id = $2::uuid`, [input.humanProfileId, input.ipProfileId])
    ...
    async beginHumanMessage(actor, input) {
      ...
      return runWithActor(actor, async (client) => {
        const inserted = await client.query<{id: string}>(
          `INSERT INTO public.chat_messages (conversation_id, role, body, delivery_state, client_request_id,generation_state,generation_answer)
           VALUES ($1::uuid, 'human', $3, 'pending', $2::uuid,'generating','')
           ON CONFLICT (conversation_id, client_request_id) DO NOTHING
           RETURNING id`,
          [input.conversationId, input.requestId, body],
        )
        const existing = await client.query<MessageRow>(
          `SELECT message.id, message.role, message.body, message.delivery_state,message.generation_state,message.generation_answer, ${utcTimestamp('message.created_at')} AS created_at,
        ...
        if (human.delivery_state === 'failed') {
          // A provider failure can occur after billing. Replaying the same
          // command must not start another generation with an uncertain outcome.
          return {type: 'failed' as const}
        }
        const reply = await client.query<MessageRow>(
          `SELECT id, role, body, delivery_state,in_reply_to_client_request_id, ${utcTimestamp('created_at')} AS created_at
           FROM public.chat_messages
    ...
    async completeProviderReply(actor, input) {
      ...
      return runWithActor(actor, async (client) => {
        ...
        const sentHuman = await client.query<MessageRow>(
          `UPDATE public.chat_messages SET delivery_state = 'sent',generation_state='completed',generation_answer=$2 WHERE id = $1::uuid AND delivery_state = 'pending'
           RETURNING id, role, body, delivery_state,client_request_id,generation_state,generation_answer, ${utcTimestamp('created_at')} AS created_at`,
          [pending.id,answer],
        )
        ...
        const insertedAssistant = await client.query<MessageRow>(
          `INSERT INTO public.chat_messages (conversation_id, role, body, delivery_state, in_reply_to_client_request_id, provider_message_id)
           ...
           RETURNING id, role, body, delivery_state,in_reply_to_client_request_id, ${utcTimestamp('created_at')} AS created_at`,
    ...
    async failHumanMessage(actor, input) {
      return runWithActor(actor, async (client) => {
        const result = await client.query<{id: string}>(
          `UPDATE public.chat_messages SET delivery_state = 'failed',generation_state='failed',generation_answer=COALESCE(generation_answer,'')
    ...
    async checkpointProviderReply(actor,input) {
      const answer=ChatBodySchema.parse(input.answer)
      return runWithActor(actor,async client=>{
        const result=await client.query(
          `UPDATE public.chat_messages SET generation_state='partial',generation_answer=$3
           WHERE id=$1::uuid AND conversation_id=$2::uuid AND role='human' AND delivery_state = 'pending'
             AND left($3,char_length(COALESCE(generation_answer,'')))=COALESCE(generation_answer,'')
           RETURNING id`,[input.humanMessageId,input.conversationId,answer])
        return result.rows.length===1
      })
    },
  }
