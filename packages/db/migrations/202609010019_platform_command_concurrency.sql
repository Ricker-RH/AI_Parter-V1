-- Forward-only hardening for platform commands: lock every authorization and
-- publishability row used for a decision until the command transaction ends.

CREATE OR REPLACE FUNCTION public.platform_create_ip(
...
BEGIN
  ...
  IF requested_languages IS NULL
    OR cardinality(requested_languages) > 20
    OR EXISTS (SELECT 1 FROM unnest(requested_languages) language WHERE language NOT IN ('en', 'zh-CN'))
  THEN RAISE EXCEPTION 'invalid IP languages' USING ERRCODE='23514'; END IF;
  ...
  VALUES(revision_id,ip_id,1,requested_display_name,requested_bio,requested_languages,operator_id);
...
CREATE OR REPLACE FUNCTION public.platform_publish_post(
...
BEGIN
  ...
  JOIN public.ip_profiles target_ip ON target_ip.profile_id=target.author_profile_id
  JOIN public.ip_identity_revisions target_revision
    ON target_revision.id=target_ip.current_identity_revision_id
    AND target_revision.ip_profile_id=target_ip.profile_id
  WHERE target.id=target_post_id AND target.state='published' AND target_ip.public_state='published'
  FOR UPDATE OF target, target_ip, target_revision;
  IF NOT FOUND THEN RAISE EXCEPTION 'published post not found' USING ERRCODE='P0002'; END IF;
