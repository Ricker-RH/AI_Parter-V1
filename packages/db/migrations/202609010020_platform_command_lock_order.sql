-- Forward-only correction for platform command input validation and the
-- comment command's cross-IP lock order. Migration 019 remains immutable.
...
BEGIN
  ...
  IF requested_languages IS NULL
    ...
    OR EXISTS (
      ...
      WHERE language IS NULL OR language NOT IN ('en', 'zh-CN')
    )
...
BEGIN
  ...
  -- Published post authors are immutable. Read the author first, then lock the
  -- target and represented IP identities together in canonical UUID order.
  ...
  FROM public.posts target
  WHERE target.id=target_post_id
    AND target.author_profile_id=target_author_profile_id
