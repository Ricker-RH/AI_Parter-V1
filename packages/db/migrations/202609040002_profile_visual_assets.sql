...
CREATE TABLE public.profile_asset_upload_reservations (
  ...
  CONSTRAINT profile_asset_reservations_object_key_length_check CHECK (
    char_length(object_key) BETWEEN 1 AND 512
  ),
  CONSTRAINT profile_asset_reservations_content_type_check CHECK (
  ...
  CONSTRAINT profile_asset_reservations_object_key_check CHECK (
    object_key = format(
      'public/profiles/%s/%s/%s.%s',
      owner_profile_id,
      role,
      id,
      CASE content_type
...
BEGIN
  ...
  expected_extension := CASE requested_content_type
    WHEN 'image/jpeg' THEN 'jpg'
    WHEN 'image/png' THEN 'png'
    WHEN 'image/webp' THEN 'webp'
  END;
  ...
  IF requested_asset_id IS NULL
    ...
    OR requested_object_key IS DISTINCT FROM expected_object_key THEN
  ...
  INSERT INTO public.profile_asset_upload_reservations (
    id, owner_profile_id, role, object_key, content_type, declared_size_bytes,
    width, height, expires_at
  ) VALUES (
    requested_asset_id, actor_profile_id, requested_role, requested_object_key,
    requested_content_type, requested_size_bytes, requested_width, requested_height,
    clock_timestamp() + interval '10 minutes'
  );

  RETURN QUERY
  SELECT r.id, r.owner_profile_id, r.role, r.object_key, r.content_type,
...
CREATE FUNCTION public.profile_confirm_asset(requested_asset_id uuid)
RETURNS TABLE(
  asset_id uuid,
  owner_profile_id uuid,
  role public.profile_asset_role,
...
BEGIN
  ...
  WHERE r.id = requested_asset_id
    ...
    AND r.consumed_at IS NULL
    AND r.expires_at > clock_timestamp()
  RETURNING r.id, r.owner_profile_id, r.role, r.object_key, r.content_type,
            r.declared_size_bytes, r.width, r.height, r.expires_at, r.verified_at;
END;
$$;
...
BEGIN
  ...
  IF change_avatar AND requested_avatar_asset_id IS NOT NULL THEN
    SELECT r.object_key INTO selected_avatar_key
    FROM public.profile_asset_upload_reservations r
  ...
  IF change_background THEN
    ...
    ELSIF requested_background_type = 'image' THEN
      SELECT r.object_key INTO selected_background_key
      FROM public.profile_asset_upload_reservations r
...
REVOKE ALL ON FUNCTION public.profile_confirm_asset(uuid) FROM PUBLIC, aifans_anon;
...
GRANT EXECUTE ON FUNCTION public.profile_reserve_asset(uuid,public.profile_asset_role,text,text,integer,integer,integer) TO aifans_authenticated;
GRANT EXECUTE ON FUNCTION public.profile_get_asset_reservation(uuid) TO aifans_authenticated;
GRANT EXECUTE ON FUNCTION public.profile_confirm_asset(uuid) TO aifans_authenticated;
