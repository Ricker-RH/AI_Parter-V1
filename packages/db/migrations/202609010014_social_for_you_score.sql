-- Stable For You formula (all terms are absolute, so a cursor never decays):
--   published_at epoch hours
--   + IP feed_weight
--   + 100 when the current viewer follows the author
--   + 10 for an exact requested locale match
--   + 2 per real post like
--   + 3 per currently published comment.
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = '' AS $$
  SELECT
    (
      extract(epoch FROM post.published_at) / 3600
      + ip.feed_weight
      + CASE WHEN EXISTS (
