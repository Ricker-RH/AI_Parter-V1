...
describe('trusted realtime publisher',()=>{
  it('publishes bounded AI history invalidations through the same authenticated gateway',async()=>{
    const fetcher=vi.fn<typeof fetch>().mockResolvedValue(new Response(null,{status:204}))
    const publisher=createRealtimePublisher({baseUrl:'https://realtime.example',secret:'s'.repeat(32),fetcher})
    const ai={v:1 as const,type:'ai_generation' as const,eventId:profileId,conversationId:profileId,messageId:profileId,state:'completed' as const,occurredAt:new Date().toISOString()}
    await publisher.publish(profileId,ai)
    expect(JSON.parse(fetcher.mock.calls[0]![1]!.body as string)).toEqual(ai)
  })
