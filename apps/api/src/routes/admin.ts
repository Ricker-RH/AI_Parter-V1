...
async function strictBody<T>(c: ApiContext, schema: z.ZodType<T>): Promise<T | null> {
  ...
  try {
    ...
    if (typeof parsedJson !== 'object' || parsedJson === null || Array.isArray(parsedJson) || hasDuplicateRootKeys(text.trim())) return null
...
function dependencyError(c: ApiContext, error: unknown): Response | null {
  ...
  if (message && /^DATABASE_(?:USER|ADMIN|PLATFORM)_URL must be a valid postgres URL$/.test(message)) {
    return apiError(c, 503, 'DATABASE_NOT_CONFIGURED', 'Database is not configured')
  }
  return null
}

async function requireOperator(c: ApiContext, dependencies: AdminDependencies): Promise<OperatorResolution> {
  const {auth, profiles, authority} = dependencies
  if (!auth) return {ok: false, response: apiError(c, 503, 'AUTH_NOT_CONFIGURED', 'Authentication is not configured')}
  ...
  if (!profiles) return {ok: false, response: apiError(c, 503, 'PROFILE_NOT_CONFIGURED', 'Profiles are not configured')}
  ...
  if (!authority) return {ok: false, response: apiError(c, 503, 'AUTHORITY_NOT_CONFIGURED', 'Authority is not configured')}
  if (!(await authority.isCurrentActorOperator(actor))) {
    return {ok: false, response: apiError(c, 403, 'OPERATOR_REQUIRED', 'An active operator is required')}
  }
  return {ok: true, actor}
}

function platformError(c: ApiContext, error: unknown, operation: PlatformOperation): Response {
  const unavailable = dependencyError(c, error)
  if (unavailable) return unavailable
  const code = errorProperty(error, 'code')
  const message = errorProperty(error, 'message')
  if (code === '42501' || message === 'FORBIDDEN') {
    return apiError(c, 403, 'OPERATOR_REQUIRED', 'An active operator is required')
  }
  ...
  if (operation === 'comment' && (code === '23514' || message === 'COMMENT_INVALID')) {
...
export function registerAdminRoutes(app: Hono<{Variables: ApiVariables}>, dependencies: AdminDependencies) {
  app.post('/v1/admin/ips', async (c) => {
    ...
    try {
      ...
      if (!dependencies.platformSocial) return apiError(c, 503, 'PLATFORM_SOCIAL_NOT_CONFIGURED', 'Platform social features are not configured')
      const result = await dependencies.platformSocial.createIp({actor: operator.actor, requestId: c.get('requestId'), ip: body})
  ...
  app.post('/v1/admin/posts', async (c) => {
    ...
    try {
      ...
      if (!operator.ok) return operator.response
      if (!dependencies.platformSocial) return apiError(c, 503, 'PLATFORM_SOCIAL_NOT_CONFIGURED', 'Platform social features are not configured')
      const result = await dependencies.platformSocial.publishPost({actor: operator.actor, requestId: c.get('requestId'), post: body})
  ...
  app.post('/v1/admin/posts/:postId/comments', async (c) => {
    ...
    if (!parsedPostId.success) return apiError(c, 400, 'INVALID_REQUEST', 'Request is invalid')
