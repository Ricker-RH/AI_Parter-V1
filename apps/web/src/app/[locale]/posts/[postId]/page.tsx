...
export default async function PostPage({params, searchParams}: {params: Promise<{locale: string; postId: string}>; searchParams: Promise<{commentCursor?: string}>}) {
  ...
  const [result,account] = await Promise.all([fetchPost(postId, {commentCursor, token: access.token}),fetchCurrentAccountResult({token: access.token})])
  if (result.status === 'auth-required' || account.status === 'auth-required') redirectToUserSignIn({locale, returnTo})
