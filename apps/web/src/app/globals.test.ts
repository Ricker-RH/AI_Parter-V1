...
describe('ordinary-user fluid shell CSS contract', () => {
  ...
  it('uses the compact mobile Home hierarchy with two equal-width feed tabs', () => {
    ...
    expect(stylesheet).toMatch(/\.mobile-nav \{[^}]*height: var\(--mobile-bottom-nav-height\)/)
    expect(stylesheet).toMatch(/\.mobile-nav \{[^}]*padding: 0 max\(8px, env\(safe-area-inset-left\)\) var\(--mobile-safe-area-bottom\)/)
  ...
  it('uses one bounded mobile public-shell viewport with an in-flow safe-area navigation row', () => {
    expect(stylesheet).toMatch(/--app-viewport-height:\s*var\(--app-visible-height, 100vh\)/)
    expect(stylesheet).toMatch(/@supports \(height: 100dvh\) \{\s*:root \{[^}]*--app-viewport-height:\s*var\(--app-visible-height, 100dvh\)/)
    ...
    expect(stylesheet).toMatch(/\.shell:is\(\[data-shell="public"\],\[data-shell="messages"\]\) \.mobile-nav \{[^}]*bottom: auto[^}]*position: static/)
  ...
  it('keeps one adaptive detail composer inline on desktop and fixed above mobile navigation', () => {
    ...
    expect(stylesheet).toMatch(/@media \(max-width: 699px\) \{[\s\S]*?\.post-detail-scroll-region\s*\{[^}]*--social-scroll-bottom-clearance:[^}]*scroll-padding-bottom:\s*var\(--social-scroll-bottom-clearance\)/)
    ...
    expect(stylesheet).toMatch(/@media \(max-width: 699px\) \{[\s\S]*?\.post-detail-scroll-region::after\s*\{[^}]*height:\s*var\(--social-scroll-bottom-clearance\)/)
