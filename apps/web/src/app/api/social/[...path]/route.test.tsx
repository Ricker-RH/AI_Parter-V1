...
describe('same-origin social mutation proxy', () => {
  ...
  it('forwards authenticated owner collection reads without accepting extra query keys', async()=>{process.env.AIFANS_API_URL='https://internal-api.example';const upstream=vi.fn().mockImplementation(async()=>Response.json({items:[],nextCursor:null}));vi.stubGlobal('fetch',upstream);for(const collection of ['likes','bookmarks']){const response=await GET(new Request(`https://web.example/api/social/${collection}`),{params:Promise.resolve({path:[collection]})});expect(response.status).toBe(200);expect(upstream).toHaveBeenLastCalledWith(`https://internal-api.example/v1/${collection}`,expect.objectContaining({headers:{authorization:'Bearer signed-jwt'}}))}expect((await GET(new Request('https://web.example/api/social/likes?admin=true'),{params:Promise.resolve({path:['likes']})})).status).toBe(400)})
