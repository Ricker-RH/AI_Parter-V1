...
export function CreatorImages({locale}:{locale:Locale}) {
  ...
  return <main className={styles.workspace}>
    ...
    <section className={styles.generation}>
      ...
      {drafts.length?<><label>{zh?'角色草稿':'Character draft'}<select value={selected} disabled={state==='working'||state==='queued'} onChange={event=>{setSelected(event.target.value);setResult(undefined);setState('idle')}}>{drafts.map(draft=><option key={draft.id} value={draft.id}>{draft.displayName}</option>)}</select></label><button type="button" onClick={generate} disabled={!selected||state==='working'||state==='queued'||state==='unavailable'}>{state==='working'?(zh?'正在提交…':'Submitting…'):(zh?'生成图片':'Generate images')}</button></>:state==='idle'?<Link href={`/${locale}/creator/studio`}>{zh?'先创建并保存一个角色 →':'Create and save a character first →'}</Link>:null}
