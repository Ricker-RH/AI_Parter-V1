...
export function CreatorCenter({locale,labels}:{locale:Locale;labels:CreatorLabels}){
  const [drafts,setDrafts]=useState<CreatorDraft[]>([]);const [ips,setIps]=useState<CreatorIp[]>([]);const [state,setState]=useState<'loading'|'ready'|'error'>('loading');const [creating,setCreating]=useState(false);const [targetIp,setTargetIp]=useState<string|undefined>()
  useEffect(()=>{let active=true;(async()=>{try{const draftPage=CreatorDraftPageSchema.parse(await creatorJson('drafts'));const ipPage=CreatorIpPageSchema.parse(await creatorJson('ips'));if(active){setDrafts(draftPage.items);setIps(ipPage.items);setState('ready')}}catch{if(active)setState('error')}})();return()=>{active=false}},[])
  return <main className="creator-page"><header className="creator-hero"><p>{labels.eyebrow}</p><div><h1>{labels.title}</h1><button onClick={()=>setCreating(true)} type="button">{labels.newIdentity}</button></div><p>{labels.description}</p></header>
    ...
    {ips.length?<section aria-labelledby="creator-identities"><h2 className="creator-section-title" id="creator-identities">{labels.identities}</h2>{ips.map((ip)=><div className="creator-ip" key={ip.id}><header><div><strong>{ip.displayName}</strong><span>@{ip.username}</span></div><span>{ip.status}</span></header><CreatorAnalytics ipProfileId={ip.id} labels={labels}/><CreatorRequestActions ipProfileId={ip.id} labels={labels} onChangeDraft={()=>{setTargetIp(ip.id);setCreating(true)}}/></div>)}</section>:null}
  </main>
}
