...
export function CreatorCenter({locale,labels,returnTo,workspace=false}:{locale:Locale;labels:CreatorLabels;returnTo?:string;workspace?:boolean}){
  ...
  return <main className={workspace?styles.workspace:'creator-page'}>{workspace?<CreatorHeader locale={locale} title={zh?'角色创作':'Character studio'} back={`/${locale}/creator`}/>:<CreatorHero labels={labels} locale={locale} {...(returnTo?{returnTo}:{})} {...(state==='ready'&&!creating?{onNewIdentity:()=>setCreating(true)}:{})}/>}
    ...
    {workspace&&tab==='create'?<footer className={styles.footer}><p className={styles.pending}>{zh?'创建功能准备中，当前输入尚未保存。':'Creation is being prepared. Your current input is not saved.'}</p></footer>:null}
    </div>
  </main>
}
