...
export function GlobalMoreMenu({authenticated, contactHref, labels, locale, onSignOut}: {authenticated?: boolean; contactHref?: string; labels: MoreMenuLabels; locale: Locale; onSignOut?: () => void}) {
  ...
  return <div className="global-more"><button aria-controls={menuId} aria-expanded={open} aria-haspopup="menu" className="more-trigger" onClick={() => open ? close() : openMenu()} onKeyDown={(event) => { if (event.key === 'ArrowDown' || event.key === 'ArrowUp') { event.preventDefault(); openMenu(event.key === 'ArrowUp' ? 'last' : 'first') } }} ref={trigger} type="button">{labels.more}</button>{open ? <div aria-label={labels.more} className="global-more-menu" id={menuId} onKeyDown={navigateMenu} ref={menu} role="menu"><Link href={`/${locale}/settings#appearance`} onClick={close} role="menuitem">{appearance}</Link><Link href={`/${locale}/settings`} onClick={close} role="menuitem">{settings}</Link>{contactHref ? <a href={contactHref} role="menuitem">{contact}</a> : <><button disabled role="menuitem" type="button">{contact}</button><span role="status">{unavailable}</span></>}{sessionState ? <button onClick={() => void signOut()} role="menuitem" type="button">{signOutLabel}</button> : null}</div> : null}</div>
}
