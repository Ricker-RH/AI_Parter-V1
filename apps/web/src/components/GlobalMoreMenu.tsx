...
export function GlobalMoreMenu({authenticated, contactHref, labels, locale, onSignOut}: {authenticated?: boolean; contactHref?: string; labels: MoreMenuLabels; locale: Locale; onSignOut?: () => void}) {
  const [open, setOpen] = useState(false); const trigger = useRef<HTMLButtonElement>(null); const menu = useRef<HTMLDivElement>(null); const menuId = useId()
  ...
  function navigateMenu(event: ReactKeyboardEvent<HTMLDivElement>) { const items = [...(menu.current?.querySelectorAll<HTMLElement>('[role="menuitem"]:not([disabled])') ?? [])]; const current = items.indexOf(document.activeElement as HTMLElement); if (!items.length) return; const destination = event.key === 'Home' ? 0 : event.key === 'End' ? items.length - 1 : event.key === 'ArrowDown' ? (current + 1 + items.length) % items.length : event.key === 'ArrowUp' ? (current - 1 + items.length) % items.length : -1; if (destination >= 0) { event.preventDefault(); items[destination]?.focus() } }
  ...
  return <div className="global-more"><button aria-controls={menuId} aria-expanded={open} className="more-trigger" onClick={() => setOpen((value) => !value)} ref={trigger} type="button">{labels.more}</button>{open ? <div aria-label={labels.more} className="global-more-menu" id={menuId} onKeyDown={navigateMenu} ref={menu} role="menu"><Link href={`/${locale}/settings#appearance`} onClick={close} role="menuitem">{appearance}</Link><Link href={`/${locale}/settings`} onClick={close} role="menuitem">{settings}</Link>{contactHref ? <a href={contactHref} role="menuitem">{contact}</a> : <><button disabled role="menuitem" type="button">{contact}</button><span role="status">{unavailable}</span></>}{sessionState ? <button onClick={() => void signOut()} role="menuitem" type="button">{signOutLabel}</button> : null}</div> : null}</div>
}
