...
vi.mock('./ChatComposer.js', () => ({ChatComposer: ({conversationId, messages}: {conversationId: string; messages: {body: string}[]}) => {
  composerRenders.push({conversationId, bodies: messages.map((message) => message.body)})
  return null
}}))

const labels = {back: 'Back', emptyHistory: 'No messages yet', loadEarlierMessages: 'Load earlier messages', messagePlaceholder: 'Write a message…', send: 'Send', sending: 'Sending…', messageFailed: 'Message failed', retry: 'Retry', providerUnavailable: 'Provider unavailable', invalidResponse: 'Invalid response', unavailable: 'Unavailable'}
const first = {conversation: {id: '11111111-1111-4111-8111-111111111111', ipProfile: {id: '22222222-2222-4222-8222-222222222222', displayName: 'Luma', username: 'luma'}, lastMessage: null, updatedAt: '2026-09-01T00:00:00.000Z', sendEnabled: true}, items: [{id: '33333333-3333-4333-8333-333333333333', role: 'assistant' as const, body: 'First history', deliveryState: 'sent' as const, createdAt: '2026-09-01T00:00:00.000Z'}], nextCursor: null}
const second = {conversation: {...first.conversation, id: '44444444-4444-4444-8444-444444444444', ipProfile: {...first.conversation.ipProfile, displayName: 'Nova', username: 'nova'}}, items: [{id: '55555555-5555-4555-8555-555555555555', role: 'assistant' as const, body: 'Second history', deliveryState: 'sent' as const, createdAt: '2026-09-01T00:01:00.000Z'}], nextCursor: null}

afterEach(() => { vi.unstubAllGlobals(); composerRenders.length = 0 })

describe('ConversationDetail', () => {
  it('reconciles local transcript state when route history changes', async () => {
    const {rerender} = render(<ConversationDetail history={first} labels={labels} locale="en" />)
  ...
  it('never renders the previous transcript under a newly selected conversation identity', () => {
    ...
    rerender(<ConversationDetail history={second} labels={labels} locale="en" />)
    expect(composerRenders).not.toContainEqual({conversationId: second.conversation.id, bodies: ['First history']})
    expect(composerRenders).toContainEqual({conversationId: second.conversation.id, bodies: ['Second history']})
  })

  it('shows a localized visible failed marker for a failed human message', () => {
    const history = {...first, items: [{...first.items[0]!, role: 'human' as const, deliveryState: 'failed' as const}]}
    render(<ConversationDetail history={history} labels={labels} locale="en" />)
  ...
  it('loads, prepends, and de-duplicates older history without replacing newer messages', async () => {
    const newer = {...first, items: [{...first.items[0]!, id: '33333333-3333-4333-8333-333333333333', body: 'Newer', createdAt: '2026-09-01T00:02:00.000Z'}]}
    const older = {...newer, items: [{...first.items[0]!, id: '44444444-4444-4444-8444-444444444444', body: 'Older', createdAt: '2026-09-01T00:01:00.000Z'}, newer.items[0]!, newer.items[0]!], nextCursor: 'older-again'}
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(Response.json(older)))
    render(<ConversationDetail history={{...newer, nextCursor: 'older-page'}} labels={labels} locale="en" />)
    fireEvent.click(screen.getByRole('button', {name: 'Load earlier messages'}))
    expect(await screen.findByText('Older')).toBeVisible()
    expect(screen.getAllByRole('listitem').map((item) => item.textContent)).toEqual(['Older', 'Newer'])
    expect(fetch).toHaveBeenCalledWith(`/api/conversations/${first.conversation.id}/messages?cursor=older-page`, expect.objectContaining({method: 'GET'}))
  ...
  it('does not merge a late older-history response into a newly selected conversation', async () => {
    let resolve!: (response: Response) => void
    vi.stubGlobal('fetch', vi.fn().mockReturnValue(new Promise<Response>((done) => { resolve = done })))
    const oldPage = {...first, items: [{...first.items[0]!, body: 'Stale older history'}], nextCursor: null}
    const {rerender} = render(<ConversationDetail history={{...first, nextCursor: 'older-page'}} labels={labels} locale="en" />)
    fireEvent.click(screen.getByRole('button', {name: 'Load earlier messages'}))
    const init = (fetch as ReturnType<typeof vi.fn>).mock.calls[0]![1] as RequestInit
    rerender(<ConversationDetail history={second} labels={labels} locale="en" />)
    expect((init.signal as AbortSignal).aborted).toBe(true)
    resolve(Response.json(oldPage))
    await waitFor(() => expect(screen.getByText('Second history')).toBeVisible())
  ...
  it('aborts an earlier-history request when the detail unmounts', async () => {
    ...
    const {unmount} = render(<ConversationDetail history={{...first, nextCursor: 'older-page'}} labels={labels} locale="en" />)
    fireEvent.click(screen.getByRole('button', {name: 'Load earlier messages'}))
    const init = (fetch as ReturnType<typeof vi.fn>).mock.calls[0]![1] as RequestInit
    unmount()
    expect((init.signal as AbortSignal).aborted).toBe(true)
  ...
  it('restores its mounted guard after Strict Effects setup-cleanup-setup', async () => {
    const older = {...first, items: [{...first.items[0]!, id: '77777777-7777-4777-8777-777777777777', body: 'Older after strict setup', createdAt: '2026-08-31T00:00:00.000Z'}], nextCursor: null}
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(Response.json(older)))
    render(<StrictMode><ConversationDetail history={{...first, nextCursor: 'older-page'}} labels={labels} locale="en" /></StrictMode>)
    fireEvent.click(screen.getByRole('button', {name: 'Load earlier messages'}))
    expect(await screen.findByText('Older after strict setup')).toBeVisible()
  })

  it.each([401, 503])('cancels a %i earlier-history response body before leaving the active request', async (status) => {
    const cancel = vi.fn().mockResolvedValue(undefined)
    if (status === 401) vi.stubGlobal('location', {assign: vi.fn()})
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({status, ok: false, headers: new Headers(), body: {cancel}}))
    render(<ConversationDetail history={{...first, nextCursor: 'older-page'}} labels={labels} locale="en" />)
    fireEvent.click(screen.getByRole('button', {name: 'Load earlier messages'}))
