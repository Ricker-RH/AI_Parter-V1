...
const labels = {
  ...
  retry: "Retry",
  providerUnavailable: "The chat provider is unavailable.",
  invalidResponse: "The chat service returned an invalid response.",
  unavailable: "Messages are unavailable right now.",
};

afterEach(() => vi.unstubAllGlobals());
describe("ChatComposer", () => {
  it("clears an in-progress replay notice after authoritative completion", async () => {
    ...
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          Response.json({ code: "CHAT_IN_PROGRESS" }, { status: 409 }),
        ),
    );
    const view = render(
      <ChatComposer
        ...
        messages={[
          {
            ...
            generation: { state: "completed", answer: "Done" },
          },
        ]}
      />,
    );
  ...
  it("requires an explicit fresh generation after a terminal failed replay", async () => {
    let sequence = 0;
    ...
    const fetcher = vi
      ...
      .mockImplementation(() =>
        Promise.resolve(
          Response.json({ code: "CHAT_GENERATION_FAILED" }, { status: 409 }),
        ),
      );
    ...
    const restart = await screen.findByRole("button", {
      name: "Start new generation",
    });
    expect(fetcher).toHaveBeenCalledTimes(1);
    expect(screen.queryByRole("button", { name: "Retry" })).toBeNull();
    fireEvent.click(restart);
    await waitFor(() => expect(fetcher).toHaveBeenCalledTimes(2));
    expect(JSON.parse(fetcher.mock.calls[0]?.[1].body).requestId).not.toBe(
      JSON.parse(fetcher.mock.calls[1]?.[1].body).requestId,
    );
  });
    ...
    await waitFor(() =>
      expect(onMessages).toHaveBeenLastCalledWith(
        expect.arrayContaining([
          expect.objectContaining({ id: human.id, deliveryState: "sent" }),
          expect.objectContaining({ id: assistant.id, body: "Hello there" }),
        ]),
      ),
    );
  });
  ...
  it("keeps a failed human message retryable with the original request id", async () => {
    ...
    const secondRequest = (fetch as ReturnType<typeof vi.fn>).mock
      .calls[1]![1] as RequestInit;
    expect(JSON.parse(secondRequest.body as string).requestId).toBe(
  ...
  it("retains a partial answer as a failed snapshot and requires an explicit new generation", async () => {
    const human = {
      id: "22222222-2222-4222-8222-222222222222",
      role: "human",
      body: "Hello",
    ...
    fireEvent.click(screen.getByRole("button", { name: "Send" }));
    await screen.findByRole("button", { name: "Start new generation" });
    expect(onMessages).toHaveBeenLastCalledWith([
      expect.objectContaining({
        id: human.id,
        deliveryState: "failed",
        generation: { state: "failed", answer: "Partial" },
      }),
    ]);
    expect(screen.queryByRole("button", { name: "Retry" })).toBeNull();
    expect(fetch).toHaveBeenCalledTimes(1);
  ...
  it("finalizes the pending human exactly once and ignores trailing frames after assistant completion", async () => {
    ...
    await waitFor(() =>
      expect(onMessages).toHaveBeenLastCalledWith([
        expect.objectContaining({ id: human.id, deliveryState: "sent" }),
        expect.objectContaining({ id: assistant.id }),
      ]),
    );
    expect(screen.queryByRole("button", { name: "Retry" })).toBeNull();
  });
      ...
      start(controller) {
        controller.enqueue(
          new TextEncoder().encode(
            `data: ${JSON.stringify({ type: "human_message", message: human })}\n\n`,
          ),
        );
      },
    });
  ...
  it("cancels a late response without publishing when fetch resolves after unmount", async () => {
    ...
    resolve({
      ...
      body: { cancelled: false, cancel: cancelled, getReader },
  ...
  it("cancels an unauthorized response body before redirecting to full-page sign-in", async () => {
    ...
    expect(assign).toHaveBeenCalledWith(
      `/en/auth/sign-in?next=${encodeURIComponent(`/en/messages/${id}`)}`,
    );
  });
  ...
  it("cancels and releases the reader immediately after assistant completion", async () => {
    ...
    const cancel = vi.fn().mockResolvedValue(undefined),
      releaseLock = vi.fn();
    const reader = {
      read: vi.fn().mockResolvedValue({ done: false, value }),
      cancel,
      releaseLock,
    };
    vi.stubGlobal(
