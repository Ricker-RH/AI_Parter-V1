...
export type ConversationListLabels = MessagesSectionLabels & {noConversations: string; emptyDescription: string; emptyAction: string; searchLabel: string; searchPlaceholder: string; noSearchResults: string; loadMore: string; emptyHistory?: string; unavailable?: string}
...
export function ConversationList({items, labels, locale, selectedId, moreHref, unavailable = false}: {items: ChatConversationSummary[]; labels: ConversationListLabels; locale: Locale; selectedId?: string | undefined; moreHref?: string | undefined; unavailable?: boolean | undefined}) {
  ...
  return <aside aria-label={labels.title} className={styles.listPane}>
    <MessagesSectionHeader active="chat" labels={labels} locale={locale}>
      <label className={styles.searchField}><span className="sr-only">{labels.searchLabel}</span><svg aria-hidden="true" viewBox="0 0 24 24"><circle cx="11" cy="11" r="6"/><path d="m16 16 4 4"/></svg><input aria-label={labels.searchLabel} onChange={(event) => setQuery(event.currentTarget.value)} placeholder={labels.searchPlaceholder} type="search" value={query}/></label>
    </MessagesSectionHeader>
    {unavailable ? <p className={styles.listNotice} role="alert">{labels.unavailable}</p> : null}
    {!unavailable && items.length === 0 ? <div className={styles.inboxEmpty}><svg aria-hidden="true" viewBox="0 0 48 48"><path d="M10 27.5 38 10 27 38l-5.5-10.5L10 27.5Z"/><path d="m21.5 27.5 8-8"/></svg><h2>{labels.noConversations}</h2><p>{labels.emptyDescription}</p><Link href={`/${locale}`}>{labels.emptyAction}</Link></div> : null}
    {!unavailable && items.length > 0 && visibleItems.length === 0 ? <p className={styles.searchEmpty} role="status">{labels.noSearchResults}</p> : null}
