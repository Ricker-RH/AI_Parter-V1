...
export function FeedContent({result, locale, labels, empty = 'home', moreHref, feedKind, visualType, currentQuery = ''}: {result: SocialApiResult<FeedPage>; locale: Locale; labels: SocialLabels; empty?: 'home' | 'bookmarks'; moreHref?: string | undefined; feedKind?: FeedKind; visualType?: FeedVisualType; currentQuery?: string}) {
  const filters = feedKind && visualType ? <nav aria-label={labels.visualTypeFilter} className="visual-filter"><div className="tabs" role="tablist">{visualTypes.map((type) => <Link aria-selected={type === visualType} className="tab" href={visualTypeHref(locale, currentQuery, type)} key={type} role="tab">{type === 'all' ? labels.allTypes : labels[type]}</Link>)}</div></nav> : null
