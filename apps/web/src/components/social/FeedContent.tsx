...
export function FeedContent({result, locale, labels, empty = 'home', emptyActionHref, moreHref, canMutate = false, returnTo, viewerScope}: {result: SocialApiResult<FeedPage>; locale: Locale; labels: SocialLabels; empty?: 'home' | 'bookmarks' | 'liked'; emptyActionHref?: string; moreHref?: string | undefined; canMutate?: boolean; returnTo?: string; viewerScope?: string}) {
  ...
  return <div className="feed-list">{result.data.items.map((post) => <PostCard canMutate={canMutate} key={post.id} labels={labels} locale={locale} post={post} referenceTime={referenceTime} {...(returnTo ? {returnTo} : {})} {...(viewerScope ? {viewerScope} : {})} />)}{result.data.nextCursor && moreHref ? <Link className="load-more" href={moreHref}>{labels.loadMore}</Link> : null}</div>
}
