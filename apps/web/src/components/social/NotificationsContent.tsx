...
export function NotificationsContent({result, locale, labels, moreHref, viewerScope}: {result: SocialApiResult<NotificationPage>; locale: Locale; labels: SocialLabels; moreHref?: string | undefined; viewerScope?: string}) {
  ...
  return <div className="notification-list">{result.data.items.map((notification) => {
    ...
    return <div className="notification-row" key={notification.id}>{notification.postId?<Link className="notification-link" href={`/${locale}/posts/${notification.postId}`}>{content}</Link>:content}{notification.readAt===null && viewerScope?<NotificationReadButton errorLabel={labels.interactionError} label={labels.markRead} locale={locale} notificationId={notification.id} pendingLabel={labels.markingRead} viewerScope={viewerScope}/>:null}</div>
