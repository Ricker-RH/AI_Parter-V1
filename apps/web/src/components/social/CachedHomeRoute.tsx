...
type Props = {labels: SocialLabels & {forYou: string; following: string; home: string; pullToRefresh: string; refreshing: string; refreshSuccess: string}; locale: Locale}
...
export function CachedHomeRoute({labels, locale}: Props) {
  ...
  const refreshHomeFeed = useRef<() => Promise<void>>(async () => undefined)
  const registerRefresh = useCallback((refresh: () => Promise<void>) => { refreshHomeFeed.current = refresh }, [])
  const refresh = useCallback(() => refreshHomeFeed.current(), [])
  const refreshLabels = {idle: labels.pullToRefresh, refreshing: labels.refreshing, success: labels.refreshSuccess}
  ...
  if (following && (status === 'loading' || status === 'anonymous')) return <SocialSurface className="home-page" header={header} label={labels.posts} onRefresh={refresh} refreshLabels={refreshLabels}><LoadingFeed label={labels.posts}/></SocialSurface>
  if (following && status === 'unavailable') return <SocialSurface className="home-page" header={header} label={labels.posts} onRefresh={refresh} refreshLabels={refreshLabels}><CachedHomeFeed canMutate={false} initialResult={{status: 'unavailable'}} kind="following" labels={labels} locale={locale} returnTo={returnTo}/></SocialSurface>

  return <SocialSurface className="home-page" header={header} label={labels.posts} onRefresh={refresh} refreshLabels={refreshLabels}><CachedHomeFeed canMutate={Boolean(account)} {...(cursor ? {cursor} : {})} kind={following ? 'following' : 'for_you'} labels={labels} locale={locale} onRefreshReady={registerRefresh} returnTo={returnTo} {...(account ? {viewerScope: scope} : {})}/></SocialSurface>
}
