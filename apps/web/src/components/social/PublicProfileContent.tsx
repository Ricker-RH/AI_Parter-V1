...
export function PublicProfileContent({result,locale,labels,moreHref}: {result:SocialApiResult<PublicIpProfile>;locale:Locale;labels:SocialLabels;moreHref?:string}) {
  ...
  return <div><section className="public-profile"><div className="avatar" aria-hidden="true">{profile.displayName.slice(0,1)}</div><div><h1>{profile.displayName}</h1><p className="author-meta">@{profile.username} · {labels[profile.visualType]}</p>{profile.creator?<p className="creator-attribution">{labels.createdBy} @{profile.creator.username}</p>:null}</div><p>{profile.bio}</p><p>{followerCount} {labels.followers}</p>{viewerFollows===undefined?<Link href={`/${locale}/auth/sign-in`}>{labels.follow}</Link>:<ProfileFollowButton following={viewerFollows} labels={labels} profileId={profile.id}/>}</section><section aria-labelledby="profile-posts"><h2 className="section-title" id="profile-posts">{labels.posts}</h2>{posts.items.length?posts.items.map(post=><PostCard key={post.id} labels={labels} locale={locale} post={post}/>):<div className="empty"><p>{labels.homeEmptyDescription}</p></div>}{posts.nextCursor&&moreHref?<Link className="load-more" href={moreHref}>{labels.loadMore}</Link>:null}</section></div>
}
