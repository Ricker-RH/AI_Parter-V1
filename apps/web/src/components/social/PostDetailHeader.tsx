...
type DetailResource = {canonicalPath: string; postId?: never} | {canonicalPath?: never; postId: string}
type PostDetailHeaderProps = DetailResource & {
  actionsLabel?: string
  fallbackHref?: string
  labels: PostDetailHeaderLabels
  locale: Locale
  referrer?: string
  title?: string
}
...
export function PostDetailHeader({actionsLabel, canonicalPath, fallbackHref, labels, locale, postId, referrer, title}: PostDetailHeaderProps) {
  ...
  const detailTitle = title ?? labels.post
  const detailActions = actionsLabel ?? labels.postActions
  const canonicalUrl = () => new URL(canonicalPath ?? `/${locale}/posts/${postId}`, window.location.origin).toString()
  ...
  async function share() { const url = canonicalUrl(); try { if (navigator.share) { await navigator.share({title: detailTitle, url}); setStatus(labels.shareSuccess) } else await copy() } catch (error) { if (error instanceof DOMException && error.name === 'AbortError') { close(); return }; await copy(); return } close() }
  return <header className="post-detail-header"><button aria-label={labels.back} className="post-detail-back" onClick={back} type="button"><BackIcon/></button><div className="post-detail-heading"><h1 className="post-detail-title">{detailTitle}</h1></div><Link aria-label="AIFANS" className="post-detail-brand" href={`/${locale}`}><Logo showWordmark={false}/></Link><div className="post-detail-menu"><button aria-controls={menuId} aria-expanded={open} aria-haspopup="menu" aria-label={detailActions} className="post-detail-menu-trigger" onClick={() => open ? close() : openMenu()} onKeyDown={(event) => { if (event.key === 'ArrowDown' || event.key === 'ArrowUp') { event.preventDefault(); openMenu(event.key === 'ArrowUp' ? 'last' : 'first') } }} ref={trigger} type="button"><MoreIcon/></button>{open ? <div aria-label={detailActions} className="post-detail-menu-list" id={menuId} onKeyDown={navigateMenu} ref={menu} role="menu"><button onClick={() => { router.refresh(); close() }} role="menuitem" type="button">{labels.refresh}</button><button onClick={() => void copy()} role="menuitem" type="button">{labels.copyLink}</button><button onClick={() => void share()} role="menuitem" type="button">{labels.share}</button></div> : null}</div><span aria-live="polite" className="post-detail-status" role="status">{status}</span></header>
}
