...
export function PostDetailHeader({labels, locale, postId, referrer}: {labels: PostDetailHeaderLabels; locale: Locale; postId: string; referrer?: string}) {
  ...
  return <header className="post-detail-header"><button className="post-detail-back" onClick={back} type="button">{labels.back}</button><h1 className="post-detail-title">{labels.post}</h1><div className="post-detail-menu"><button aria-controls={menuId} aria-expanded={open} aria-haspopup="menu" aria-label={labels.postActions} className="post-detail-menu-trigger" onClick={() => open ? close() : openMenu()} onKeyDown={(event) => { if (event.key === 'ArrowDown' || event.key === 'ArrowUp') { event.preventDefault(); openMenu(event.key === 'ArrowUp' ? 'last' : 'first') } }} ref={trigger} type="button">•••</button>{open ? <div aria-label={labels.postActions} className="post-detail-menu-list" id={menuId} onKeyDown={navigateMenu} ref={menu} role="menu"><button onClick={() => { router.refresh(); close() }} role="menuitem" type="button">{labels.refresh}</button><button onClick={() => void copy()} role="menuitem" type="button">{labels.copyLink}</button><button onClick={() => void share()} role="menuitem" type="button">{labels.share}</button></div> : null}</div><span aria-live="polite" className="post-detail-status" role="status">{status}</span></header>
}
