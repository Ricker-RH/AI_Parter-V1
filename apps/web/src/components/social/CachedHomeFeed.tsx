...
type Props={canMutate:boolean;cursor?:string;emptyActionHref?:string;empty?:'home'|'bookmarks'|'liked';initialResult:SocialApiResult<FeedPage>;kind:'for_you'|'following';labels:SocialLabels;locale:Locale;returnTo:string;viewerScope?:string}

async function load(kind:'for_you'|'following',locale:Locale,cursor?:string):Promise<SocialApiResult<FeedPage>>{
  const params=new URLSearchParams({kind,locale})
  if(cursor)params.set('cursor',cursor)
  try{
    const response=await fetch(`/api/feed?${params}`,{cache:'no-store',credentials:'same-origin'})
...
function CachedFeed(props:Props){
  ...
  const query=useQuery({queryKey:['home-feed',scope,props.locale,props.kind,props.cursor??null],queryFn:()=>load(props.kind,props.locale,props.cursor),initialData:props.initialResult,staleTime:30_000})
  ...
  return <FeedContent canMutate={props.canMutate} {...(props.empty?{empty:props.empty}:{})} {...(props.emptyActionHref?{emptyActionHref:props.emptyActionHref}:{})} labels={props.labels} locale={props.locale} {...(moreHref?{moreHref}:{})} result={result} returnTo={props.returnTo} {...(props.viewerScope?{viewerScope:props.viewerScope}:{})}/>
}
