...
export interface ShellLabels extends MoreMenuLabels { primary: string; home: string; forYou?: string; following?: string; search: string; notifications: string; messages: string; bookmarks: string; profile: string; settings: string; creatorNav: string; recommendations: string; recommendationsEmpty: string }
...
function NavLink({item, locale, label, compact}: {item: NavItem; locale: Locale; label: string; compact?: boolean}) {
  ...
  const active = item.key === 'following' ? pathname === `/${locale}` && search.get('feed') === 'following' : pathname === href
  return <Link aria-current={active ? 'page' : undefined} aria-label={compact ? label : undefined} className="nav-link" href={href}><Icon aria-hidden="true" className="nav-icon"/><span className={compact ? 'nav-link-label sr-only' : 'nav-link-label'}>{label}</span></Link>
}
