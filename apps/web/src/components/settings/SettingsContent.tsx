...
export function SettingsContent({locale,configured}:{locale:Locale;configured:boolean}) {
 ...
 return <div className={styles.root}>
  <section className={styles.group}><h2>{zh?'账号与资料':'Account and profile'}</h2><div className={styles.card}>
   <Link className={styles.row} href={`/${locale}/profile/edit?returnTo=${encodeURIComponent(`/${locale}/settings`)}`}><span className={styles.label}>{zh?'个人资料':'Edit profile'}</span><span className={styles.value}>{zh?'头像、昵称与简介':'Avatar, name and bio'} <b aria-hidden="true">›</b></span></Link>

  </div></section>
  <section className={styles.group}><HumanPreferencesEditor locale={locale}/></section>
  <section className={styles.group}><h2>{zh?'外观与语言':'Appearance and language'}</h2><div className={styles.card}>
   <details id="appearance"><summary className={styles.row}><span className={styles.label}>{zh?'外观主题':'Theme'}</span><span className={styles.value}>{mounted&&themeName?(zh?themeName.zh:themeName.en):zh?'跟随系统':'System'} <b aria-hidden="true">›</b></span></summary><div className={styles.palettes}>{themes.map(t=><button key={t.key} type="button" aria-pressed={mounted&&theme===t.key} onClick={()=>setTheme(t.key)}><span className={styles.swatch} style={{background:t.color}} aria-hidden="true"/><span>{zh?t.zh:t.en}</span>{mounted&&theme===t.key?<span aria-hidden="true">✓</span>:null}</button>)}</div></details>
   <details><summary className={styles.row}><span className={styles.label}>{zh?'显示语言':'Display language'}</span><span className={styles.value}>{zh?'简体中文':'English'} <b aria-hidden="true">›</b></span></summary><div className={styles.options}><Link href="/zh-CN/settings" aria-current={zh?'true':undefined}>简体中文 {zh?'✓':''}</Link><Link href="/en/settings" aria-current={!zh?'true':undefined}>English {!zh?'✓':''}</Link></div></details>
  </div></section>
  <section className={styles.group}><h2>{zh?'动态效果':'Motion'}</h2><div className={styles.card}><details><summary className={styles.row}><span className={styles.label}>{zh?'动画效果':'Animations'}</span><span className={styles.value}>{motion==='reduce'?(zh?'减少动态':'Reduced'):(zh?'跟随系统':'System')} <b aria-hidden="true">›</b></span></summary><p className={styles.help}>{zh?'减少头像光圈等动画，让浏览更平静。设置仅用于当前设备。':'Reduce avatar and interface animations. Applies to this device.'}</p><div className={styles.options}>{['system','reduce'].map(value=><button key={value} type="button" aria-pressed={motion===value} onClick={()=>updateMotion(value)}>{value==='system'?(zh?'跟随系统':'Follow system'):(zh?'减少动态':'Reduce motion')} {motion===value?'✓':''}</button>)}</div></details></div></section>
