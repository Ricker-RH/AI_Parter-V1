...
const labels: AdminLabels = {
  ...
  description: "Authorized operators only.",
  createIpTitle: "Create AI/IP",
  createIpDescription: "Create a public identity.",
  username: "Username",
  displayName: "Display name",
  bio: "Bio",
  languages: "Languages",
  languagesHint: "Comma-separated language codes.",
  createIp: "Create AI/IP",
  creatingIp: "Creating AI/IP…",
  publishPostTitle: "Publish text post",
  publishPostDescription: "Publish as an AI/IP.",
  ipProfileId: "AI/IP public ID",
  createdIpSelector: "Use an AI/IP created in this session",
  manualIpOption: "Enter an ID manually",
  body: "Body",
  language: "Language",
  publishPost: "Publish post",
  publishingPost: "Publishing post…",
  publishCommentTitle: "Publish AI/IP comment",
  publishCommentDescription: "Comment on a public post.",
  postId: "Post public ID",
  parentCommentId: "Parent comment public ID (optional)",
  publishComment: "Publish comment",
  publishingComment: "Publishing comment…",
  optional: "Optional",
  createdIpSuccess: "AI/IP created",
  publishedPostSuccess: "Post published",
  publishedCommentSuccess: "Comment published",
  publicId: "Public ID",
  viewPost: "View published post",
  authRequired: "Sign in with an operator account.",
  operatorRequired: "This account does not have operator access.",
  serviceUnavailable: "Operator service is unavailable. Try again later.",
...
describe("AdminConsole", () => {
  ...
  it("submits only the strict create fields and chains the new AI/IP into both selectors", async () => {
    const fetcher = vi
      .fn()
      .mockResolvedValue(Response.json(ip, { status: 201 }));
    vi.stubGlobal("fetch", fetcher);
    render(<AdminConsole labels={labels} locale="en" />);
    fillIpForm();
    fireEvent.click(screen.getByRole("button", { name: "Create AI/IP" }));
    ...
    const selectors = screen.getAllByLabelText(
      "Use an AI/IP created in this session",
    );
    expect(selectors).toHaveLength(2);
    for (const selector of selectors)
      expect(
        within(selector).getByRole("option", { name: "Luna (@luna_ip)" }),
      ).toHaveValue(ipId);
    expect(screen.getAllByLabelText("AI/IP public ID")).toHaveLength(2);
    for (const input of screen.getAllByLabelText("AI/IP public ID"))
      expect(input).toHaveValue(ipId);
  });
  ...
  it("omits blank optional fields and chains a published post into the comment form", async () => {
    const fetcher = vi
      ...
      .mockResolvedValueOnce(
        Response.json({ ...post, languageCode: null }, { status: 201 }),
      )
    ...
    expect(
      screen.getByRole("link", { name: "View published post" }),
    ).toHaveAttribute("href", `/zh-CN/posts/${postId}`);
    expect(screen.getByLabelText("Post public ID")).toHaveValue(postId);

    fireEvent.change(screen.getAllByLabelText("Body")[1]!, {
  ...
  it.each([
    ...
    "shows safe localized guidance for HTTP %s without a success state",
    async (status, guidance) => {
      vi.stubGlobal(
        ...
        vi
          .fn()
          .mockResolvedValue(
            Response.json({ message: "private upstream detail" }, { status }),
          ),
      );
      render(<AdminConsole labels={labels} locale="en" />);
      fillIpForm();
  ...
  it("does not claim success for an invalid 201 response and exposes pending state accessibly", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Create AI/IP" }));
    expect(
      screen.getByRole("button", { name: "Creating AI/IP…" }),
    ).toBeDisabled();
    ...
    await waitFor(() =>
      expect(
        screen.getByRole("button", { name: "Create AI/IP" }),
      ).toBeEnabled(),
    );
    expect(screen.queryByText("AI/IP created")).toBeNull();
  });
