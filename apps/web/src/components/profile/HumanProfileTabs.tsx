...
function ScopedHumanProfileTabs({profile,locale,socialLabels,viewerScope}:Props){
 ...
 return <section className={styles.tabsSection}><div aria-label={labels.tabs} className={styles.tabList} role="tablist">{tabs.map(tab=>{
  ...
  {page?.tab===tab?<>{page.items.length===0?<ProfileEmptyState kind={tab} locale={locale} title={locale==='zh-CN'?'暂无内容。':'No content yet.'}/>:page.tab==='liked'||page.tab==='saved'?page.items.map(post=><PostCard canMutate={Boolean(viewerScope)} key={post.id} labels={socialLabels} locale={locale} post={post as FeedPost} referenceTime={Date.now()} returnTo={`/${locale}/humans/${profile.identity.id}`} {...(viewerScope?{viewerScope}:{})}/>):<div className={styles.ipList}>{page.items.map(item=><Link className={styles.ipRow} href={`/${locale}/${item.kind==='human'?'humans':'profiles'}/${item.id}`} key={item.id}><Avatar avatarUrl={'avatarUrl' in item?item.avatarUrl??null:null} displayName={item.displayName} decorative identityId={item.id} kind={item.kind==='ip'?'ip':'human'} size="medium"/><span><strong>{item.displayName}</strong><small>@{item.username}</small>{'bio' in item&&item.bio?<p>{item.bio}</p>:null}</span></Link>)}</div>}</>:null}
